//hex2MoT01.c wmh 2021-11-03 : generate MoT command string from console input, check for validity, print out byte interpretastion for confirmation
...
int hexcmd2bin(char *hexcmd, uint8_t *bincmd);	//processes a text buffer containing a MoT command string into a binary command buffer; 

int main(int argc, char *argv[])	 // generate MoT command string from console input (see info at top), print it, then decode it
{
	uint8_t tst;
	uint32_t bytesum=0;
	uint8_t chksum=0;
	...
	*poutbuf++ ='\0';	// EOS
	...
	//here with MoT command in outbuf -- now decode it to binary to test hexcmd2bin() function to be used on the micro
